...
def inverse_document_frequencies(tokenized_documents):
    ...
    max_count = max([term_frequency(t, tokenized_documents) for t in tokenized_documents])
    print("Max Count %d"%max_count)
    for tkn in all_tokens_set:
        print(con)
        con += 1
        contains_token = map(lambda doc: tkn in doc, tokenized_documents)
        idf_values[tkn] = (0.5 + ((0.5 * term_frequency(tkn, tokenized_documents))/max_count)) * math.log(len(tokenized_documents)/(sum(contains_token)))
